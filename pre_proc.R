...
pre_proc <- function(data_raw, unlist = TRUE) {
  ...
  library(stringr)
  players <- str_extract_all(names(df)[c(4:16,34, 36, 39, 40, 42)], "(?<=\\[).+?(?=\\])")
  players <- unlist(players)

  n_games <- length(games)
  df_res_tot <- data.frame("game" = rep(games, each = 18), "player" = rep(players, n_games))
  res <- list()
  ...
  for (i in games) {
    ...
    names(df_raw_game)[c(4:17, 34, 36, 39, 40, 42)] <- unlist(str_extract_all(names(df)[c(4:17, 34, 36, 39, 40, 42)], "(?<=\\[).+?(?=\\])"))
    ...
    # raw_scores
    scores <- df_raw_game[, c(18:30, 35, 37, 38, 41, 43)]
